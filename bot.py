...
def _ai_chat(prompt, max_tokens=1500, retries=1):
    ...
    if elapsed < _MIN_REQUEST_GAP:
        ...
        print("[Groq] Rate limit gap, wait " + str(round(wait, 1)) + "s")
        time.sleep(wait)
    for model in models:
        _last_ai_error = ""
        for attempt in range(retries + 1):
            try:
                ...
                if r.status_code == 200:
                    ...
                    print("[Groq Error] " + _last_ai_error)
                    return None
                    ...
                    print("[Groq] 429 on " + model + ", wait " + str(wait_sec) + "s")
                    if attempt < retries:
                        time.sleep(wait_sec)
                    continue
                else:
                ...
                print("[Groq Error] " + str(attempt + 1) + "/" + str(retries + 1) + " [" + model + "]: " + err_str)
            if attempt < retries and not _last_ai_error.startswith("Rate limited"):
                time.sleep(3)
        if model == models[0]:
            print("[Groq] " + models[0] + " failed, trying fallback...")
            time.sleep(5)
    return None
...
def _check_key():
    if not GROQ_API_KEY or len(GROQ_API_KEY) < 10:
        return "GROQ_API_KEY belum diatur atau tidak valid."
    return None
