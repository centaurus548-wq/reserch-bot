...
def build_report_embed(btc, dxy, gdata, fg, news, analysis, cmc_coins, cmc_global):
    ...
    if analysis:
        ...
        # Discord embed max 25 fields; existing fields before this: ~4 (Data, CMC, Global, News)
        max_ai_fields = 21
        for title, content in sections[:max_ai_fields]:
            # Discord limit: field name max 256 chars, field value max 1024 chars
            if len(title) > 256:
                title = title[:253] + "..."
            if len(content) > 1024:
...
async def report(ctx):
    ...
    try:
        ...
        pass  # msg already deleted or not found, ignore
    except discord.errors.HTTPException:
        # Embed too large or invalid — send text fallback
        try:
            await msg.edit(content="❌ Gagal mengirim report: embed terlalu besar/invalid. Coba lagi.")
        except discord.errors.NotFound:
            pass
    except Exception as e:
        try:
